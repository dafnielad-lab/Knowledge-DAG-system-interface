...
נסמן $C(n) = 1^3 + 2^3 + \cdots + n^3$ ונוכיח באינדוקציה (לפי ax_principle_of_mathematical_induction) שעבור $n \geq 1$ מתקיים $C(n) = \left(\frac{n(n+1)}{2}\right)^2$.
...
נוציא גורם משותף $(n+1)^2$ באמצעות ax_distributive_law:
...
**הערה — הקשר ל-thm_sum_first_n_naturals:** אפשר לכתוב את הזהות כ-$C(n) = \bigl(S(n)\bigr)^2$ כאשר $S(n) = 1 + 2 + \cdots + n = \frac{n(n+1)}{2}$ (לפי thm_sum_first_n_naturals), ולכן הטענה היא $\sum_{k=1}^n k^3 = \bigl(\sum_{k=1}^n k\bigr)^2$.

לפי ax_principle_of_mathematical_induction, הזהות תקפה לכל $n \geq 1$. $\blacksquare$
